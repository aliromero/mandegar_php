...
<div @include('crud::inc.field_wrapper_attributes') >
    ...
    <select
        ...
        @if (isset($field['model']))
            ...
            $data = $field['model']::join('cities', 'cities.id', '=', 'areas.city_id')
                ->select('areas.*')
                ->selectRaw('cities.name as city')
                ->get(); ?>
            @foreach ($data as $connected_entity_entry)
                <option value="{{ $connected_entity_entry->getKey() }}"
                    @if ( (isset($field['value']) && in_array($connected_entity_entry->getKey(), $field['value']->pluck($connected_entity_entry->getKeyName(), $connected_entity_entry->getKeyName())->toArray())) || ( old( $field["name"] ) && in_array($connected_entity_entry->getKey(), old( $field["name"])) ) )
                         selected
                    @endif
                >{{ $connected_entity_entry->{$field['attribute']} }} - منطقه {{ $connected_entity_entry->area_num }} {{ $connected_entity_entry->city }}</option>
            @endforeach
        @endif
    </select>
