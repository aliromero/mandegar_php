...
@section('header')
    <?php
    $shop_id = (\Request::has('shop_id')) ? '?shop_id=' . \Request::input('shop_id') : '';
    ?>
    <section class="content-header">
        <h1>
            <span class="text-capitalize">{{ $crud->entity_name_plural }}</span><br/>
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{ url($crud->route) . $shop_id }}" class="text-capitalize">{{ $crud->entity_name_plural }}</a></li>
            ...
            <div class="box">
                <div class="box-header {{ $crud->hasAccess('create')?'with-border':'' }}">
                    ...
                    @if ($crud->hasAccess('create'))
                        <a href="{{ url($crud->route.'/create?shop_id=' . $crud->shop_id) }}" class="btn btn-primary ladda-button" data-style="zoom-in"><span class="ladda-label"><i class="fa fa-plus"></i> {{ trans('backpack::crud.add') }} {{ $crud->entity_name }}</span></a>
                    @endif


                    <div id="datatable_button_stack" class="pull-left text-right"></div>
                </div>
...
register_details_row_button_action();
            ...
            function register_read_button_action() {
                $("[data-button-type=read]").unbind('click');
                // CRUD Delete
                // ask for confirmation before deleting an item
                $("[data-button-type=read]").click(function (e) {
                    e.preventDefault();
                    var read_button = $(this);
                    var read_url = $(this).attr('href');

                    if (confirm("{{ trans('backpack::crud.read_confirm') }}") == true) {
                        $.ajax({
                            url: read_url,
                            type: 'PUT',
                            data: { read: 1 },
                            success: function (result) {
                                // Show an alert with the result
                                new PNotify({
                                    title: "{{ trans('backpack::crud.read_confirmation_title') }}",
                                    text: "{{ trans('backpack::crud.read_confirmation_message') }}",
                                    type: "success"
                                });
                                // delete the row from the table
                                read_button.parentsUntil('tr').parent().find('td:first i.fa').removeClass('fa-square-o').addClass('fa-check-square-o');
                                read_button.remove();
...
//
                            ...
                            error: function (result) {
                                // Show an alert with the result
                                new PNotify({
                                    title: "{{ trans('backpack::crud.read_confirmation_not_title') }}",
                                    text: "{{ trans('backpack::crud.read_confirmation_not_message') }}",
                                    type: "warning"
                                });
                            }
                        });
                        ...
                        new PNotify({
                            title: "{{ trans('backpack::crud.read_confirmation_not_read_title') }}",
                            text: "{{ trans('backpack::crud.read_confirmation_not_read_message') }}",
                            type: "info"
                        });
                    }
                });
